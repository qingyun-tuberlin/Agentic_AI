...
def get_leakage_warning(context: callback_context_module.ReadonlyContext) -> str:
    """Aggregates target leakage/XAI warnings across all solutions.

    The ensemble combines every candidate solution, so it must surface leakage
    flags from all of them (train_code_0_1, train_code_0_2, ...), not just one.
    """
    warnings = []
    num_solutions = context.state.get("num_solutions", config.CONFIG.num_solutions)
    ...
    return (
        ...
        "Any code modification or ensemble plan MUST NOT add these features back under any circumstances.\n"
    )
