...
- MANDATORY STABILITY CONSTRAINT: The study MUST preserve the Skrub DataOps execution graph syntax (skrub.var, .skb.apply, .skb.make_learner()).
- BANNED SYNTAX: You are completely prohibited from decomposing the graph into raw scikit-learn training blocks (model.fit(X, y)).
- To isolate feature performance variations without breaking inference, alter column sets explicitly using list filters, rather than calling .drop().
...
- Dont replace skrub data ops with similar solutions.
- If an error occurs try again with skrub data ops.
...
EXTRACT_BLOCK_AND_PLAN_INSTR = strip_rag_tool_mentions(EXTRACT_BLOCK_AND_PLAN_INSTR)
EXTRACT_BLOCK_AND_PLAN_SEQ_INSTR = strip_rag_tool_mentions(EXTRACT_BLOCK_AND_PLAN_SEQ_INSTR)
IMPLEMENT_PLAN_INSTR = strip_rag_tool_mentions(IMPLEMENT_PLAN_INSTR)
