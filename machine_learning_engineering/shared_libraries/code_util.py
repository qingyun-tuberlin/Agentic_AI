...
from machine_learning_engineering.shared_libraries import xai_tracker
...
def _ensure_runtime_libraries(run_cwd: str) -> None:
    """Copy shipped helper modules next to the script before execution.

    Generated scripts run as a separate subprocess via `["python", <file>]`
    with cwd=run_cwd, so the script's own directory is on `sys.path[0]`. Placing
    `xai_probes.py` there lets the script do a bare `import xai_probes` (the same
    way it reads `./input/...`). Best-effort: a copy failure must not block the
    run — a script that does not import the helper is unaffected.
    """
    try:
        from machine_learning_engineering.shared_libraries import xai_probes

        shutil.copyfile(
            xai_probes.__file__, os.path.join(run_cwd, "xai_probes.py")
        )
    except Exception as exc:  # noqa: BLE001 - never block execution on this
        logger.warning("Could not stage xai_probes.py in %s: %s", run_cwd, exc)
...
def run_python_code(
    ...
    with open(output_filepath, "w", encoding="utf-8") as f:
        f.write(code_text)
    _ensure_runtime_libraries(run_cwd)
    try:
    ...
    # Attribute this run's wall-clock to the per-task overhead tracker. The
    # tracker decides (via its phase flag) whether it counts as XAI compute.
    xai_tracker.record_code_run(execution_time)
    result_dict = {
    ...
    for line in lines:
        if "Final Validation Performance" in line:
            try:
                ...
                # Extract float or int using a robust regex pattern
                match = re.search(r"[-+]?\d*\.\d+(?:[eE][-+]?\d+)?|[-+]?\d+(?:[eE][-+]?\d+)?", score_str)
                if match:
                    performance_value = float(match.group(0))
                else:
...
def extract_performance_line_from_text(text: str) -> str | None:
    """Return the last stdout line containing 'Final Validation Performance'."""
    last_line = None
...
def evaluate_code(
    ...
    if get_run_code_condition(
        ...
        if agent_name.startswith("ablation"):
            ...
            # Fallback logic for submission: if validation score was omitted, statically infer it
            if agent_name.startswith("submission") and (score is None or score == (1e9 if lower else 0)):
                try:
                    import difflib
                    def clean_code(s):
                        if not s:
                            return ""
                        return "\n".join(line.strip() for line in s.splitlines() if line.strip() and not line.strip().startswith("#"))
                    sub_clean = clean_code(raw_code)
                    best_k = None
                    best_ratio = 0.0
                    for k, v in callback_context.state.items():
                        if ("code" in k) and not k.endswith("_exec_result") and isinstance(v, str) and k != "submission_code" and len(v) > 100:
                            cand_clean = clean_code(v)
                            ratio = difflib.SequenceMatcher(None, cand_clean, sub_clean).ratio()
                            if ratio > best_ratio:
                                best_ratio = ratio
                                best_k = k
                    if best_ratio >= 0.7 and best_k:
                        exec_k = None
                        if best_k.startswith("train_code_improve_"):
                            parts = best_k.split("_")
                            suffix_part = "_".join(parts[3:])
                            exec_k = f"train_code_improve_exec_result_{suffix_part}"
                        elif best_k.startswith("train_code_"):
                            parts = best_k.split("_")
                            suffix_part = "_".join(parts[2:])
                            exec_k = f"train_code_exec_result_{suffix_part}"
                        elif best_k.startswith("ensemble_code_"):
                            parts = best_k.split("_")
                            suffix_part = parts[2]
                            exec_k = f"ensemble_code_exec_result_{suffix_part}"
                        if exec_k and exec_k in callback_context.state:
                            ref_exec = callback_context.state[exec_k]
                            if isinstance(ref_exec, dict) and "score" in ref_exec:
                                ref_score = ref_exec["score"]
                                if ref_score is not None and ref_score != (1e9 if lower else 0):
                                    result_dict["score"] = ref_score
                                    result_dict["stdout"] = (result_dict.get("stdout") or "") + f"\nFinal Validation Performance: {ref_score}\n"
                except Exception:
                    pass
    else:
        result_dict = {}
