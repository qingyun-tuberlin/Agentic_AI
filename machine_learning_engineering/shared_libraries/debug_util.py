...
def get_debug_agent_instruction(
    ...
    # Detect execution timeouts and inject specific simplification guidance
    code_execution_result_state_key = (
        code_util.get_code_execution_result_state_key(
            agent_name=agent_name,
            suffix=suffix,
    ...
    if "timed out" in stderr.lower() or "timeoutexpired" in stderr.lower():
        exec_timeout = context.state.get("exec_timeout", 1800)
        instruction += debug_prompt.TIMEOUT_GUIDANCE.format(
...
def get_code_from_response(
    ...
    if agent_name.startswith("check_data_use"):
        ...
        if "debug" not in agent_name:
            ...
            if code_block and code_block in prev_code:
